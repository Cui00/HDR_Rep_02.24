...
def HDR(_imgs_nx1, _times_nx1, method=Debevec):
    ...
    if method == Debevec:
        ...
        hdr_img = merge_debvec.process(src=_imgs_nx1,  times=_times_nx1)
    ...
    if method == Robertson:
        ...
        hdr_img = merge_robertson.process(src=_imgs_nx1,  times=_times_nx1)
        #local tonermap
        tonemap = cv2.createTonemapDurand(gamma=1.4)
        res_img = tonemap.process(hdr_img.copy())
        return crf, hdr_img, res_img
    if method == Mertens:
        ...
        # cv2.imwrite("PyFusion1.png", res_mertens_8bit)
        return res_img

def showSaveData(crf, res_img, txt_name, png_name):
    tmp = crf.reshape(256, 3)
    np.savetxt('../text/'+txt_name, tmp, fmt='%.2f')
    #draw response curve
...
if __name__ == "__main__":
    ...
    crf, hdr_img, res_img = HDR(imgs_nx1, times_nx1, )
    showSaveData(crf, res_img, "crf_debevec.txt", "figure_debevec.png")
    # result_img = HDR(imgs_nx1, times_nx1, method=Mertens)
    ...
    # crf, hdr_img, res_img = HDR(imgs_nx1, times_nx1, method=Robertson)
    # showSaveData(crf, res_img, "crf_robertson.txt", "figure_robertson.png")
    pass
